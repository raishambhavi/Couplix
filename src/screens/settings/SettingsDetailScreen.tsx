...
import type { SettingsStackParamList } from './settingsNavPrimitives';
import type { SettingsPanelId } from './settingsNavPrimitives';

function calendarDayKey(ms: number | null | undefined): string {
...
export function SettingsDetailScreen() {
  ...
  const panel: SettingsPanelId = route.params.panel;
  ...
  const [birthDateDraftMs, setBirthDateDraftMs] = useState<number | null>(auth.profile?.dateOfBirthMs ?? null);
  const [birthPickerShow, setBirthPickerShow] = useState(false);

  useEffect(() => {
    setDisplayNameDraft(auth.profile?.displayName ?? '');
    setEmailDraft(auth.profile?.email ?? '');
    setPhoneDraft(auth.profile?.phoneNumber ?? '');
    setBirthDateDraftMs(auth.profile?.dateOfBirthMs ?? null);
  }, [auth.profile?.displayName, auth.profile?.email, auth.profile?.phoneNumber, auth.profile?.dateOfBirthMs]);
  useEffect(() => {
  ...
  const profileDirty =
    ...
    calendarDayKey(birthDateDraftMs) !== calendarDayKey(auth.profile?.dateOfBirthMs);
  ...
  const saveProfileChanges = async () => {
    ...
    try {
      ...
      if (calendarDayKey(birthDateDraftMs) !== calendarDayKey(auth.profile?.dateOfBirthMs)) {
        await auth.setDateOfBirthMs(birthDateDraftMs);
      }
      ...
      <View style={styles.screenRoot}>
        ...
        <SoftCard>
          <View style={styles.section}>
            ...
            <View style={styles.profileRow}>
              <Text style={[styles.rowLabel, { color: colors.text }]}>Full Name</Text>
            ...
            <View style={styles.profileRow}>
              <Text style={[styles.rowLabel, { color: colors.text }]}>Date of Birth</Text>
              <View style={styles.profileActionRow}>
                <Pressable
                  onPress={() => {
                    setBirthPickerShow(true);
                    setProfileSavedNote('');
                  }}
                  style={[styles.inlineInput, { borderColor: colors.border, justifyContent: 'center' }]}
                >
                  <Text style={{ color: birthDateDraftMs != null ? colors.text : colors.muted, fontWeight: '600' }}>
                    {birthDateDraftMs != null
                      ? new Date(birthDateDraftMs).toLocaleDateString(undefined, {
                          year: 'numeric',
                          month: 'short',
                          day: 'numeric',
                        })
                ...
                <Pressable
                  onPress={() => setBirthDateDraftMs(auth.profile?.dateOfBirthMs ?? null)}
            ...
            {birthPickerShow ? (
              <DateTimePicker
                value={birthDateDraftMs != null ? new Date(birthDateDraftMs) : new Date(1995, 0, 1)}
                mode="date"
                ...
                onChange={(ev, date) => {
                  if (Platform.OS === 'android') setBirthPickerShow(false);
                  if (ev.type === 'dismissed' && Platform.OS === 'android') return;
                  if (date) {
                    const normalized = new Date(date.getFullYear(), date.getMonth(), date.getDate());
                    setBirthDateDraftMs(normalized.getTime());
                    setProfileSavedNote('');
                  }
                }}
              />
            ) : null}
            {Platform.OS === 'ios' && birthPickerShow ? (
              <GoldButton title="Done" onPress={() => setBirthPickerShow(false)} style={{ marginTop: 8 }} />
            ) : null}
        ...
        <SoftCard>
          <View style={styles.section}>
            ...
            <Text style={[styles.rowLabel, { color: colors.text }]}>Hobbies</Text>
            <TextInput
              value={hobbies}
            ...
            <Text style={[styles.rowLabel, { color: colors.text, marginTop: 4 }]}>How I pass my time</Text>
            <TextInput
              value={passTimeBy}
              onChangeText={setPassTimeBy}
              placeholder="How I pass my time"
              placeholderTextColor={colors.muted}
              style={[styles.input, { color: colors.text, borderColor: colors.border }]}
            />

            <Text style={[styles.rowLabel, { color: colors.text, marginTop: 4 }]}>Favourite destination</Text>
            <TextInput
            ...
            <Text style={[styles.rowLabel, { color: colors.text, marginTop: 4 }]}>Cuisine I love</Text>
            <Text style={[styles.pairHint, { color: colors.muted, marginTop: 2 }]}>
              Choose one or more — opens a list you can tap to select.
            </Text>
            <Pressable
              onPress={() => setCuisineModalOpen(true)}
              style={[styles.partnerPick, { borderColor: colors.border, alignSelf: 'stretch', marginTop: 6 }]}
            >
              <Text style={[styles.partnerPickText, { color: selectedCuisines.length ? colors.text : colors.muted }]}>
                {selectedCuisines.length
                  ? selectedCuisines.join(', ')
                  : 'Tap to choose cuisines (multiple)'}
              </Text>
            </Pressable>
        ...
        <SoftCard>
          <View style={styles.section}>
            ...
            <GoldButton
              title={passwordResetSending ? 'Sending…' : 'Change password'}
              disabled={passwordResetSending}
              onPress={async () => {
                setPasswordResetSending(true);
                try {
                  await auth.sendPasswordResetToAccountEmail();
                  Alert.alert(
                    'Check your email',
                    'We sent a password reset link to the email on this Couplix account. Open it on this device to choose a new password.'
                  );
                } catch (e: unknown) {
                  const msg =
                    e && typeof e === 'object' && 'message' in e ? String((e as { message: string }).message) : String(e);
                  Alert.alert('Could not send reset email', msg || 'Please try again.');
                } finally {
            ...
            <Text style={[styles.accountHint, { color: colors.muted }]}>
              Uses your account email (same as in Manage profile). You’ll set the new password from the link Firebase sends.
            </Text>
        ...
        <Modal visible={cuisineModalOpen} transparent animationType="fade" onRequestClose={() => setCuisineModalOpen(false)}>
          <Pressable style={styles.modalBackdrop} onPress={() => setCuisineModalOpen(false)}>
            <Pressable style={[styles.modalCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Text style={[styles.modalTitle, { color: colors.text }]}>Cuisine I love</Text>
              <Text style={[styles.pairHint, { color: colors.muted, textAlign: 'center', marginTop: 6 }]}>
                Tap to select or deselect. Then tap outside or Done.
              </Text>
                    ...
                    <Pressable
                      ...
                      style={[
                        styles.modalLine,
                        {
                          ...
                          backgroundColor: on ? 'rgba(231,199,125,0.12)' : 'transparent',
                        },
                      ]}
                    >
                      <Text style={[styles.modalLineText, { color: colors.text }]}>{c}</Text>
                      {on ? <Ionicons name="checkmark-circle" size={22} color={colors.gold} /> : null}
                    </Pressable>
                  );
                })}
              </ScrollView>
              <GoldButton title="Done" onPress={() => setCuisineModalOpen(false)} style={{ marginTop: 14, alignSelf: 'stretch' }} />
            </Pressable>
          </Pressable>
        </Modal>
